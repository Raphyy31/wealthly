...
from app.schemas import (
    ...
    GoogleSignInRequest, AuthConfigOut,
)
...
def auth_config():
    """Public endpoint — returns non-secret configuration the frontend needs
    to initialise third-party auth providers (e.g. Google client_id).
...
def google_sign_in(request: Request, response: Response, payload: GoogleSignInRequest, db: Session = Depends(get_db)):
    """Verify a Google Identity Services credential (id_token) and return a
    JWT session cookie. Creates a new account automatically on first sign-in."""
    import os
    import httpx

    if not settings.GOOGLE_CLIENT_ID:
        raise HTTPException(status_code=501, detail="Connexion Google non configurée sur ce serveur.")

    # Verify id_token with Google's tokeninfo endpoint (no client secret needed).
    try:
        r = httpx.get(
        ...
        raise HTTPException(status_code=502, detail="Impossible de vérifier le token Google. Réessayez.")

    # Validate audience to prevent token confusion attacks.
    aud = info.get("aud") or info.get("azp") or ""
    if aud != settings.GOOGLE_CLIENT_ID:
        record_auth_event(db, kind="google_login_failure", success=False, request=request,
                          detail="aud_mismatch")
    ...
    if not google_id or not email:
        raise HTTPException(status_code=400, detail="Informations Google incomplètes.")

    # Find user: first by google_id (stable), then by email (links existing account).
    ...
    if not user:
        ...
        db.commit()
        db.refresh(user)
        record_auth_event(db, kind="google_register_success", success=True, request=request,
                          user_id=user.id, email=user.email)
    else:
        record_auth_event(db, kind="google_login_success", success=True, request=request,
                          user_id=user.id, email=user.email)
